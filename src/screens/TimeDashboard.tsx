...
const CustomTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-3 shadow-xl text-xs">
      <p className="font-semibold text-gray-800 mb-2">{label} · {formatHours(total)}</p>
      {payload.map((p: any) => p.value > 0 && (
        <div key={p.dataKey} className="flex items-center gap-2 py-0.5">
          <span className="w-2 h-2 rounded-full shrink-0" style={{ background: p.fill }} />
          <span className="text-gray-600">{p.name}</span>
          <span className="ml-auto text-gray-900 font-medium pl-4">{formatHours(p.value)}</span>
        </div>
      ))}
    </div>
  );
...
export default function TimeDashboard() {
  ...
  const categoryTotals = useMemo(() => {
    ...
    filtered.forEach(e => { totals[e.category] = (totals[e.category] ?? 0) + e.hours; });
    return totals;
  }, [filtered]);
  ...
  const chartData = useMemo(() => {
    if (period === '1uge') {
      ...
      return weeks.map(weekStart => {
        ...
        const ws = toISO(weekStart); const we = toISO(wEnd);
    ...
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex items-start justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">{formatDate(start)} – {formatDate(end)}</p>
        </div>
        <PeriodSelector
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
          ...
          <p className="text-2xl font-bold text-blue-600">{formatHours(invoicedHours)}</p>
          {totalHours > 0 && <p className="text-xs text-gray-400 mt-0.5">{Math.round((invoicedHours / totalHours) * 100)}% af total</p>}
        </div>
        <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
          <p className="text-xs text-gray-500 mb-1 flex items-center gap-1"><MapPin size={11} /> On-site</p>
          <p className="text-2xl font-bold text-orange-500">{formatHours(onSiteHours)}</p>
          {totalHours > 0 && <p className="text-xs text-gray-400 mt-0.5">{Math.round((onSiteHours / totalHours) * 100)}% af total</p>}
        </div>
      </div>
          ...
          return (
            <div key={cat} className="bg-white border border-gray-200 rounded-xl p-3 shadow-sm">
              <div className="flex items-center gap-1.5 mb-2">
                ...
                <span className="text-xs text-gray-500 truncate">{getCategoryLabel(cat)}</span>
              </div>
              <p className="text-xl font-bold text-gray-900">{formatHours(h)}</p>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5 mb-6 shadow-sm">
        <h2 className="text-sm font-medium text-gray-700 mb-4">Timer pr. {period === '1uge' ? 'dag' : 'uge'}</h2>
        <ResponsiveContainer width="100%" height={280}>
          <BarChart data={chartData} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
            ...
            <Legend formatter={(value: string) => <span style={{ color: '#6b7280', fontSize: 12 }}>{getCategoryLabel(value as TimeCategory)}</span>} />
            {ALL_CATEGORIES.map(cat => (
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
        <div className="p-4 border-b border-gray-200">
          <h2 className="text-sm font-medium text-gray-700">Seneste registreringer</h2>
        </div>
        {filtered.length === 0 ? (
          <p className="text-center py-8 text-sm text-gray-400">Ingen registreringer i perioden</p>
        ) : (
          <div className="divide-y divide-gray-100">
            {[...filtered].sort((a, b) => b.date.localeCompare(a.date)).slice(0, 20).map(entry => {
              ...
              return (
                <div key={entry.id} className="flex items-center gap-3 px-4 py-2.5 text-sm hover:bg-gray-50">
                  <span className="text-gray-400 w-22 shrink-0 text-xs">{entry.date}</span>
                  <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ background: color }} />
                  <span className="text-gray-500 text-xs shrink-0">{getCategoryLabel(entry.category)}</span>
                  <span className="text-gray-700 flex-1 truncate">
                    {c?.title ?? '—'} <span className="text-gray-400">· {client?.name ?? '—'}</span>
                  </span>
                  {entry.description && (
                    <span className="text-gray-400 text-xs truncate max-w-[200px]">{entry.description}</span>
                  )}
                  <span className="font-semibold text-gray-900 shrink-0">{formatHours(entry.hours)}</span>
                </div>
              );
            })}
          </div>
